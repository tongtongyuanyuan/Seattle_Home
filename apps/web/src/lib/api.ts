...
export interface OpenHouse {
  ...
  why_pick_2?: string;
  why_pick_3?: string;
  tags?: string; // comma-separated: "value,commute,schools"
...
export async function fetchOpenHouses(params?: {
  ...
  // Filter by area if specified (match area OR address, case-insensitive)
  if (params?.area && params.area !== 'All') {
    const area = params.area.toLowerCase();
    results = results.filter(
      (house) =>
        house.area.toLowerCase().includes(area) ||
        house.address.toLowerCase().includes(area)
    );
  }

  // Filter by day if specified ("Saturday" should match "Sat 1-4 PM")
  if (params?.day && params.day !== 'All') {
    const day = params.day.slice(0, 3).toLowerCase();
    results = results.filter((house) =>
      house.open_house_time.toLowerCase().includes(day)
    );
  }
